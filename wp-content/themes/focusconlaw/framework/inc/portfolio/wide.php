<div class="portfolio-wide clearfix">
	...
	<?php } else { ?>
	    ...
	    if (get_post_meta( get_the_ID(), 'minti_source', true ) == 'vimeo') {  
	        echo '<div id="portfolio-video" class="sixteen columns"><iframe src="http://player.vimeo.com/video/'.get_post_meta( get_the_ID(), 'minti_embed', true ).'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" width="960" height="540" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';  
	    }  
	    else if (get_post_meta( get_the_ID(), 'minti_source', true ) == 'youtube') {  
	        echo '<div id="portfolio-video" class="sixteen columns"><iframe width="960" height="540" src="http://www.youtube.com/embed/'.get_post_meta( get_the_ID(), 'minti_embed', true ).'?rel=0&showinfo=0&modestbranding=1&hd=1&autohide=1&color=white" frameborder="0" allowfullscreen></iframe></div>';  
	    }  
	    else {  
	        echo '<div id="portfolio-video" class="sixteen columns">'.get_post_meta( get_the_ID(), 'minti_embed', true ).'</div>'; 
	    }  
	    ?>
	    
	<?php } ?>
		
	<div class="portfolio-detail-description <?php if( get_post_meta( get_the_ID(), 'minti_portfolio-details', true ) == 1 ) { echo 'eleven'; } else { echo 'sixteen'; } ?> columns">
